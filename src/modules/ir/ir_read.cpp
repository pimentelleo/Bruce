...
int button_pos = 0;
static char *quickButtonsTV[] = {"POWER", "UP",   "DOWN", "LEFT", "RIGHT", "OK",       "SOURCES",
                                 "VOL+",  "VOL-", "MUTE", "CHA+", "CHA-",  "SETTINGS", "NETFLIX",
                                 "HOME",  "BACK", "EXIT", "1",    "2",     "3",        "4",
                                 "5",     "6",    "7",    "8",    "9",     "0"};
static char *quickButtonsAC[] = {"POWER", "TEMP+", "TEMP-", "LIGHT", "FANSPEED", "SWING", "FANONOFF", "MODE"};
static char *quickButtonsSOUND[] = {"POWER",      "VOL+", "VOL-",  "TUN+",        "TUN-",
                                    "PLAY/PAUSE", "STOP", "EJECT", "SEEKFORWARD", "SEEKBACKWARD",
                                    "MUTE",       "MODE", "RADIO", "CD",          "AUX",
                                    "1",          "2",    "3",     "4",           "5",
                                    "6",          "7",    "8",     "9",           "0"};
static char *quickButtonsPROJECTOR[] = {"POWER",  "UP",   "DOWN", "LEFT",   "RIGHT", "OK",       "SOURCES",
                                        "VOL+",   "VOL-", "MUTE", "CHA+",   "CHA-",  "SETTINGS", "ZOOM",
                                        "MENU",   "BACK", "EXIT", "LIGHTS", "VGA",   "HDMI",     "VIDEO",
                                        "RESYNC", "6",    "7",    "8",      "9",     "0"};
...
void IrRead::setup() {
    ...
    std::vector<Option> quickOptions = {
        {"TV",
         [&]() {
             quickloop = true;
             begin(quickButtonsTV);
             return loop(quickButtonsTV);
         }},
        {"AC",
         [&]() {
             quickloop = true;
             begin(quickButtonsAC);
             return loop(quickButtonsAC);
         }},
        {"SOUND",
         [&]() {
             quickloop = true;
             begin(quickButtonsSOUND);
             return loop(quickButtonsSOUND);
         }},
        {"PROJECTOR",
         [&]() {
             quickloop = true;
             begin(quickButtonsPROJECTOR);
             return loop(quickButtonsPROJECTOR);
         }},
    };
    ...
    options = {
        ...
        {"Quick Remote Setup  ", [&]() { loopOptions(quickOptions); }},
        {"Menu",                 []() {}                             },
    };
    loopOptions(options);

    loopOptions(quickOptions);
}

void IrRead::loop(char *buttons[]) {
    while (1) {
        if (check(EscPress)) {
            returnToMenu = true;
            button_pos = 0;
        ...
        if (check(SelPress) || button_pos == (sizeof(buttons) / sizeof(buttons[0]))) save_device();
...
void IrRead::begin(char *buttons[]) {
    _read_signal = false;

    display_banner();
    if (quickloop) {
        padprintln("Waiting for signal of button: " + String(buttons[button_pos]));
    } else {
